...
seriesRouter.delete('/:seriesId', (req, res, next) => {
  ...
  db.all('SELECT * FROM Issue WHERE series_id = $seriesId', {$seriesId: seriesId}, (err, rows) => {
    if (err) {
      return next(err);
    }
    if (rows.length > 0) {
    ...
    db.run('DELETE FROM Series WHERE id = $seriesId', {$seriesId: seriesId}, function(err) {
      if (err) {
        return next(err);
      }
